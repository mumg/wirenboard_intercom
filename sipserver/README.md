...
### Внешние UAC-аккаунты

Для регистрации на сторонних SIP-серверах используйте секцию `external_accounts`.

Основные поля:

- `name`: имя аккаунта для логов;
- `interface`: локальный интерфейс из `interfaces`, через который будет идти `REGISTER` и приём входящих вызовов;
- `server`: адрес стороннего SIP-сервера в формате `host:port`;
- `domain`: SIP-домен для `From`/`To`/AOR;
- `username`, `auth_username`, `password`: учётные данные внешнего аккаунта;
- `contact_user`: user-часть `Contact`, на которую сторонний сервер будет присылать входящие вызовы;
- `caller`: локальный пользователь, от имени которого сервер будет инициировать внутренний обзвон;
- `callee`: локальный внутренний номер или `distributed_extension`, который нужно обзвонить;
- `expires`: срок внешней регистрации в секундах.

Текущее поведение:

- сервер сам поддерживает `REGISTER` на внешний SIP-сервер в фоне;
- входящий `INVITE` на такой UAC обрабатывается как обычный входящий звонок с media proxy и early media;
- для внутреннего обзвона берутся именно `caller` и `callee` из конфига, а не значения из внешнего `From`/`To`;
- сейчас для `external_accounts` поддерживается только `UDP`.
